...
def read_config():
    ...
    if 'SETTINGS' in config:
        ...
        region = config['SETTINGS'].get('region', 'default_region')
        region_visibility = config['SETTINGS'].getboolean('region_visibility', True)
    else:
    ...
    return base_dir, last_promotion_time, path_visibility, region, region_visibility

# Функция для записи конфигурации
def write_config(base_dir, last_promotion_time, path_visibility, region, region_visibility):
    config = configparser.ConfigParser()
    config['SETTINGS'] = {
        'base_dir': base_dir,
        ...
        'region': region,
        'region_visibility': str(region_visibility)
    }
    with open(config_file, 'w') as configfile:
        config.write(configfile)
# Основные категории
categories = ["Таблетки", "ПМП", "ПМП ОБ", "Мед. Карты", "Уколы", "МП-ГМП-АКЦИИ"] if read_config()[4] == 'об' else ["Таблетки", "ПМП", "ПМП ЛС", "Мед. Карты", "Уколы", "МП-ГМП-АКЦИИ"]
        ...
        if read_config() == 'об':
            if any(place in text4.lower() for place in
                   ['занкудо', 'палето', 'чилиад', 'гора', 'джосайя', 'гордо', 'пустыня', 'гранд', 'сенора',
                    'сан-шаньский', 'шорс', 'северный чумаш', 'грейпсид', 'брэддока', 'хармони', 'ратон', 'аламо', 'кэтфиш',
            ...
            if any(place in text4.lower() for place in [
                'мишн', 'роу', 'пиллбокс', 'хилл', 'текстайл', 'сити', 'площадь', 'легиона',
                'центр', 'вайнуда', 'восточный', 'вайвуд', 'станция', 'миррор', 'парк',
                'сортировочная', 'станция', 'миррор', 'парк', 'гоночная', 'трасса', 'вайвуд',
                'западный', 'вайвуд', 'альта', 'хавик', 'южный', 'лос', 'сантос',
                'дэвис', 'строберри', 'чемберлен', 'хиллз', 'ранчо',
                'порт', 'лос', 'сантоса', 'баннинг', 'элизан', 'айленд', 'терминал',
                'восточный', 'лос', 'сантос', 'ла', 'меса', 'сайпресс', 'флэтс',
                'эль', 'бурро', 'хайтс', 'мурьета', 'хайтс', 'веспуччи',
                'веспуччи', 'бич', 'каналы', 'веспуччи', 'пуэрто', 'дель', 'соль',
                'бэко', 'сити', 'richards', 'majestic', 'productions',
                'дель', 'перро', 'дель', 'перро', 'бич', 'ла', 'пуэрта',
                'maze', 'bank', 'arena', 'маленький', 'сеул', 'международный', 'аэропорт', 'лос', 'сантос',
                'морнингвуд', 'пасифик', 'блаффс', 'центр', 'кодз', 'римчан', 'хиллз',
                'пакк', 'хиллз', 'винвуд', 'хиллз', 'галилео', 'парк', 'озеро', 'винвуд',
                'lake', 'vinewood', 'estates', 'римчан', 'хиллз', 'театр', '"синфулус"',
                'vinewood', 'bowl', 'каньон', 'банхэм', 'банхэм', 'каньон', 'драйв', 'чумаш',
                'тонгва', 'хиллз', 'долина', 'тонгва', 'грейп', 'чапарал',
                'каньон', 'байкёр', 'татавиамские', 'горы', 'горькое',
                'ледяной', 'резервару', 'нагорья', 'папомино', 'штаб', 'квартира', 'нуп'
            ]):
                return "ПМП ЛС"
            else:
                return None
...
# Основная функция
def main():
    base_dir, last_promotion_time, path_visibility, region, region_visibility = read_config()
    ...
    print("Чтобы отключить вопрос о регионе, введите '2' в вопросе о пути или дате.")

    if path_visibility:
        get_valid_input_1 = get_valid_input(f"Текущий путь к скриншотам: {base_dir}. Хотите обновить? (y/n/1/2): ", lambda x: x.lower() in ['y', 'n', '1', '2'])
        ...
        elif get_valid_input_1 == '2':
            region_visibility = False

    get_valid_input_2 = get_valid_input(f"Текущая дата последнего повышения: {last_promotion_time.strftime('%Y-%m-%d %H:%M')}. Хотите обновить? (y/n/1/2): ", lambda x: x.lower() in ['y', 'n', '1', '2'])
    if get_valid_input_2 == 'y':
        ...
        path_visibility = True
    elif get_valid_input_2 == '2':
        region_visibility = False

    if region_visibility:
        get_valid_input_3 = get_valid_input(f"Текущий регион: {region}. Хотите обновить? (y/n): ", lambda x: x.lower() in ['y', 'n'])
        if get_valid_input_3 == 'y':
            region = get_valid_input("Введите новый регион (лс/об): ", lambda x: x.lower() in ['лс', 'об'])

    write_config(base_dir, last_promotion_time, path_visibility, region, region_visibility)
